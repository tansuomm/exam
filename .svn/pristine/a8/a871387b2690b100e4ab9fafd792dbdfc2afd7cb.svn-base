...
//带参数的人员窗体
function ClerkDialogAppendPar(RootUrl,ClerkIDElement,ClerkNameElement,MinItem,MaxItem,par)
{
	var url = RootUrl + "Admin/SelectStudent.aspx";
	url += "?ClerkIDs=" + document.getElementById(ClerkIDElement).value;
	url += "&ClerkNames=" + escape(document.getElementById(ClerkNameElement).value);
	...
	var returnValue = window.showModalDialog(url + par,window,'dialogWidth:520px;dialogHeight:540px;status: no;scroll: no');
	if(returnValue != undefined) 
	{
	document.getElementById(ClerkIDElement).value = returnValue[0];
	document.getElementById(ClerkNameElement).value = returnValue[1];
...
{
	//定义变量 = 页面.创建一个元素 Option 下拉每个选项
    var oOption = document.createElement("OPTION");
    //下拉框.options.添加 刚才创建的option
...
function ButtonSetPageIndexLimit(maxPage)
{
    if(parseInt(maxPage)< parseInt(document.getElementById("TbPageNum").value))
    {
        alert("页码超出范围!");
        return false;
    }
...
//函数
//作用 选择课程分类的ID
//参数 detpIDElement 课程分类ID号 detpNameElement 课程分类
function departTree( detpIDElement,detpNameElement)
{
  var url = "../SelectDetp.aspx";
	url += "?detpID=" + document.getElementById(detpIDElement).value;
	url += "&detpIDElement="+detpIDElement;
	url += "&detpNameElement="+detpNameElement;
	window.open(url, null, "width=280,height=430,top=60,left=710");	 	
}
function departTreeByPath(path,detpIDElement,detpNameElement)
{
  var url =  path + "SelectDetp.aspx";
	url += "?detpID=" + document.getElementById(detpIDElement).value;
	url += "&detpIDElement="+detpIDElement;
	url += "&detpNameElement="+detpNameElement;
	window.open(url, null, "width=280,height=430,top=60,left=710");
